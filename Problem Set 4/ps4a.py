...
def getWordScore(word, n):
    ...
    for letter in word:
        if letter in SCRABBLE_LETTER_VALUES:
            point.append(SCRABBLE_LETTER_VALUES[letter])
    for i in point:
        total_point += i
    total_point *= len(word)
    if len(word) == n:
        total_point += 50
    print(total_point)
    return total_point
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while letters_left > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            elif isValidWord(user_input, hand, wordList):
                # Tell the user how many points the word earned, and the updated total score, in one line followed by a blank line
                total_score += getWordScore(user_input,n)
                print( '" ' + user_input + ' "' + ' earned: ' + str(getWordScore(user_input,n)) + '. Total: ' + str(total_score) +'\n')        
...
def playGame(wordList):
    ...
    track = {}
    while True:
        user_request = input("Enter n to deal a new hand, r to replay the last hand, or e to end game:")
        if user_request == 'n':
            n = HAND_SIZE
            hand = dealHand(n)
            track.setdefault('n',hand)
